...
test('СУМКА ПОЛНА: золото не списывается за воздух', () => {
  ...
  assert.equal(res.ok, false);
  if (!res.ok) assert.match(res.reason, /full/);
});
...
test('ВЕЩЬ В ЯЧЕЙКЕ СМЕНИЛАСЬ — продажа отклонена, ничего не тронуто', () => {
  // Дыра из состязательной проверки: игрок отобрал стальной меч, горячая
  // клавиша за тот же кадр положила в ячейку снятый синий — по голому номеру
  // продался бы синий, которого игрок не выбирал.
  const b: (Stack | null)[] = [{ id: 'sword_blue', qty: 1 }, null];
  const res = sellStack(100, b, 0, 'sword'); // ожидали стальной
  assert.equal(res.ok, false);
  assert.deepEqual(b[0], { id: 'sword_blue', qty: 1 }, 'синий меч цел');

  const okRes = sellStack(100, b, 0, 'sword_blue'); // совпало — продаётся
  assert.ok(okRes.ok);
